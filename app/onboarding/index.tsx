...
import { ALL_THEME_KEYS, type ContentKinds, type ThemeKey } from "@/domain/types";
...
/**
 * Onboarding with a STICKY footer: the step's primary "continue" button is
 * pinned below a scrollable content area, so it is visible from the first
 * frame of every step — the earlier single-scroll layout buried it under
 * the theme grid and the schedule pickers, and finishing setup meant
 * hunting for the button at the bottom of a long page.
 */
export default function OnboardingScreen(): React.JSX.Element {
  ...
  const toggleContentKind = (kind: keyof ContentKinds): void => {
    const nextKinds = { ...preferences.contentKinds, [kind]: !preferences.contentKinds[kind] };
    if (!nextKinds.ayah && !nextKinds.hadith && !nextKinds.name && !nextKinds.dua) return;
    update({ contentKinds: nextKinds });
  };
  ...
  // The one always-visible progression button for the current step.
  const footer =
    step === 1 ? (
  ...
  return (
    <Screen scroll={false} contentContainerStyle={{ padding: 0, flex: 1 }}>
      <View style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: spacing.md, paddingBottom: spacing.lg, gap: spacing.md }} showsVerticalScrollIndicator={false}>
          ...
          {step === 6 ? (
            <View style={{ gap: spacing.md }}>
              ...
              <Button label={t("onboarding.step6.sendTestCta")} variant="secondary" onPress={runTest} />
  ...
  function toggleTheme(theme: ThemeKey): void {
    ...
    const nextThemes = has ? preferences.selectedThemes.filter((th) => th !== theme) : [...preferences.selectedThemes, theme];
    update({ selectedThemes: nextThemes });
  }
}
