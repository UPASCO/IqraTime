...
## 99 Names of Allah and Invocations (2.0.0)

Two further datasets follow the same never-typed-from-memory rule as the
Qur'an and hadith corpora. Both are generated by
`scripts/fetchNamesAndDuas.mjs`, which fetches, cross-validates, and
writes provenance notes (`_readme`) into the generated files.

**`src/data/names/names.json`** — the 99 Names (Al-Asma ul-Husna) in the
canonical (Tirmidhi) order. Arabic and English meanings are copied
verbatim from the MIT-licensed
[imanikurd](https://github.com/Abdulla090/imanikurd) dataset, and the
...
selected) are copied verbatim — Arabic, transliteration, English
...
as the hadith corpus's language gaps.

The daily rotations are deterministic and communal — same name and same
dua for every user on the same local calendar day (`getDailyNameNumber`,
`getDailyDuaId`) — mirroring `dailyAyah.ts`.
