# Stripe Setup — "Support AyahNow"

AyahNow's optional donation feature has no server component. It opens a
...
```
AyahNow  →  system browser (Safari / Chrome / secure custom tab)  →  Stripe Payment Link  →  Stripe  →  the bank account configured in Stripe
```
...
- **No Stripe secret key, PaymentSheet, or Payment Intents API is used in
  ...
  securely — AyahNow has no server, and a secret key must never ship
...
- **No local server, no proxy, no webhook receiver.** AyahNow cannot and
...
7. Name the product/contribution **"Soutenir AyahNow"** (or "Support
   AyahNow" for an English-facing link — Stripe Payment Links support one
...
- It cannot verify a payment succeeded — there is no server, no webhook.
  ...
  supporting AyahNow" message after the browser opens; it never claims
